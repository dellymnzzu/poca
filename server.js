...
MongoCilent.connect(
  "mongodb+srv://admin:dlsgud5031@cluster0.7igrfxz.mongodb.net/poca?retryWrites=true&w=majority",
  (에러, client) => {
...
app.get("/mypage/profile", 로그인했니, (req, res) => {
  db.collection("login")
...
app.post("/like", (req, res) => {
  ...
  if (Data === data) {
    ...
    db.collection("like").findOne(
      { 찜아이디: req.user.id, 게시물번호: parseInt(req.body._id) },
      (error, result) => {
        db.collection("content")
          ...
          .toArray((error, result2) => {
            ...
            if (result) {
              ...
              db.collection("like").insertOne(저장, (error, result) => {}); //늦게 동작해도 상관 없으면 괜찬, 아니면 방법 생각

              db.collection("content").updateOne(
                { _id: parseInt(req.body._id) },
                { $addToSet: { likeid: data }, $inc: { 좋아요: 1 } },
                (result) => {}
              );
            }
          });
